...
# model_kwargs={"temperature": 0.5, "max_length": 256, "raw_response":True})
...
# llm = ChatOpenAI(
#     model_name="gpt-4-turbo",  # Use GPT-4 Turbo for faster and cheaper inference
#     temperature=0.01,
#     openai_api_key=os.getenv("OPENAI_API_KEY")
# )
...
# llm = VertexAI(
#     model_name="gemini-pro",  # Or try "gemini-1.0-pro" or "gemini-pro-vision" for image + text
...
def sql_copilot(language_model=llm, max_attempts=3):
    ...
    for schema in list(top_schemas): #its in a set, so have to convert to a list
        ...
        print(db.get_usable_table_names()) #debugging line to see the tables in the database
        ...
        while attempt <= max_attempts:
            print("I am trying!!!", query_to_run)
